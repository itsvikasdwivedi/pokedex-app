...
const Pokemon = () => {
    ...
    return (
        <div className=" grid gap-4 sm:grid-cols-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 px-5 py-10">
            {paginatedData && paginatedData?.map((pokemon) => (
                ...
                <div className='hover:-translate-y-3 p-5 border-2 border-grey-500 rounded-2xl shadow-md hover:border-violet-300 md:h-100 lg:h-100 2xl:h-100' key={pokemon.id}>
                    ...
                    <div >
                        ...
                        <h2 className='font-mono text-xl ml-1 mt-2' >{pokemon.name}</h2>

                        <p className='ml-1 mt-2'>{pokemon.types.join(' ')}</p>
                    </div>
                </div>
                    </Link>
            ))}
